...
static char				*reverse_dns_lookup()
{
	...
	status = getnameinfo((struct sockaddr *) &temp_addr, sizeof(struct sockaddr_in), buff_hostname, NI_MAXHOST, NULL, 0, NI_NAMEREQD);
	if (status != 0)
	{
...
static int				set_ip_addr(t_ping *g_ping)
{
	...
	while (addr_ptr)
	{
		...
		{
			ft_memcpy(&(g_ping->ip4), (void *)addr_ptr->ai_addr, sizeof(struct sockaddr_in));
			addr_str = inet_ntop(AF_INET, &(g_ping->ip4.sin_addr), g_ping->ip_addr, INET_ADDRSTRLEN);
			break ;
		}
